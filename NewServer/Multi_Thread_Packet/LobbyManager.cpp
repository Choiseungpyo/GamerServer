...
set<int> LobbyManager::reusableRoomNos;
...
void  LobbyManager::CreateRoom(const Packet* packet, const ClientSession* client)
{
	...
	Room* newRoom = new Room(roomNo, roomOption, pack->id, client);
...
{	
	...
	UpdateInRoomInfo(client, S_C_INROOM_INFO, room, pack->roomNo);
...
void LobbyManager::SetReadyState(const ClientSession* client)
{
	...
	// 방장이 시작했을 경우
	if (roomUserInfo->isHost == userId && room->IsAllReady())
	{
		room->CreateNewGame();
		UpdateLobbyRoomInfo(room);
		return;
...
void LobbyManager::DeleteRoom(int roomNO)
{
	auto it = roomMap.find(roomNO);
	if (it == roomMap.end()){
		cout << "Delete Room - Invalid roomId : " << roomNO << endl;
		return;
	}

	// 삭제한 방번호 저장 : 새로 생성할 때 해당 공간에 채우기 위함
	reusableRoomNos.insert(roomNO);
